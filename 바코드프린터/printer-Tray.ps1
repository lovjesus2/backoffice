...
$startInfo.Arguments = "printer-server.js"
...
try {
    ...
    Write-Host "PRINTER Server started in: $scriptDir"
} catch {
    Write-Host "Failed to start PRINTER Server: $($_.Exception.Message)"
    exit 1
}
...
$tray.Text = "PRINTER-SERVER Agent"
$tray.Visible = $true
...
$status.Text = "PRINTER-SERVER Running"
...
$renewCert.Add_Click({
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
        try {
            ...
            [System.Windows.Forms.MessageBox]::Show("Certificate renewed successfully! Please restart PRINTER-SERVER Agent.", "Success")
        } catch {
...
$tray.BalloonTipTitle = "PRINTER-SERVER Agent"
if ($certInfo.Status -eq "Warning") {
    $tray.BalloonTipText = "PRINTER-SERVER Agent started in $scriptDir`nWarning: Certificate expires in $($certInfo.DaysLeft) days!"
    $tray.BalloonTipIcon = [System.Windows.Forms.ToolTipIcon]::Warning
} elseif ($certInfo.Status -eq "OK") {
    $tray.BalloonTipText = "PRINTER-SERVER Agent started in $scriptDir`nCertificate valid for $($certInfo.DaysLeft) days"
    $tray.BalloonTipIcon = [System.Windows.Forms.ToolTipIcon]::Info
} else {
    $tray.BalloonTipText = "PRINTER-SERVER Agent started in $scriptDir`nCertificate issue: $($certInfo.Message)"
