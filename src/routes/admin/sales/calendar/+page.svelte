...
<script>
  ...
  function handleStockUpdated(event) {
    ...
    // ✅ salesGroups는 dailySalesDetail에서 자동 생성되므로 별도 업데이트 불필요
    // $: salesGroups = groupSalesBySlip(dailySalesDetail); 로 자동 반응
  }
...
function handleStockUsageUpdated(event) {
  ...
  dailySalesDetail = dailySalesDetail.map(item => 
    item.pcode === productCode 
      ? { ...item, isStockManaged: stockManaged }  // ← 올바른 변수명!
      : item
  );
}
  ...
  function handleCashStatusUpdated(event) {
    ...
    dailySalesDetail = dailySalesDetail.map(item => 
      item.pcode === productCode 
        ? { ...item, cash_status, hasPresetCashPrice: cash_status }
        : item
    );
    dailySalesDetail = [...dailySalesDetail];
  }
