...
<script>
 ...
 // import { stateManager } from '$lib/utils/stateManager.js';   이전상태 복원
  ...
  let componentLoadingInProgress = new Set(); // 새로 추가

  $: {
    
    $openPages.forEach(async (href) => {
      // 이미 로딩 중이거나 완료된 것은 스킵
      if (componentLoadingInProgress.has(href) || loadedComponents[href] || !pageComponents[href]) {
        return;
      }
      
      componentLoadingInProgress.add(href); // 로딩 시작 표시
      
      try {
        const module = await pageComponents[href]();
        loadedComponents[href] = module.default;
        loadedComponents = { ...loadedComponents };
        console.log('✅ 컴포넌트 로딩 완료:', href);
      } catch (error) {
        console.error(`Failed to load component for ${href}:`, error);
      } finally {
        componentLoadingInProgress.delete(href); // 로딩 완료 표시
      }
    });
  }
  ...
  */
  ...
  $: if (browser && $page.url.pathname && $page.url.pathname !== '/') {
    // 이미 열려있는 페이지면 중복 호출 방지
    if (!$openPages.has($page.url.pathname)) {
  ...
  if (browser && 'Notification' in window && Notification.permission === 'granted') {
    ...
    console.log('ℹ️ 알림 권한이 없거나 브라우저가 지원하지 않음');
  }
