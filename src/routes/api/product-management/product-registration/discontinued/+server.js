// src/routes/api/product-management/product-registration/discontinued/+server.js
import { json } from '@sveltejs/kit';
import { getDb } from '$lib/database.js';

export async function POST({ request, locals }) {
  try {
    console.log('=== Product Registration Toggle Discontinued API 호출 시작 ===');
    ...
    const { product_code, company_code = 'A1', registration_code = 'AK', registration_name = '' } = await request.json();
    
    console.log('단종 처리 요청:', { 
      product_code, 
      company_code, 
      registration_code, 
      registration_name,
    ...
    // 제품정보가 아닌 경우 단종 처리 불가
    if (registration_name !== '제품정보') {
      return json({
        success: false,
        message: '제품정보 등록구분에서만 단종 처리가 가능합니다.'
      }, { status: 400 });
    }
    
    const db = getDb();
    
    // 제품 정보 조회 (현재 단종 상태 확인) - 동적 회사구분/등록구분 적용
    const [productRows] = await db.execute(`
      ...
      WHERE p.PROH_GUB1 = ? AND p.PROH_GUB2 = ? AND p.PROH_CODE = ?
    ...
    try {
      // 1. ASSE_PROD 테이블 업데이트 (L5: 단종 상태) - 동적 회사구분/등록구분 적용
      await db.execute(`
        UPDATE ASSE_PROD 
        SET PROD_TXT1 = ?, PROD_IDAT = NOW(), PROD_IUSR = ?
        WHERE PROD_GUB1 = ? AND PROD_GUB2 = ? 
        AND PROD_CODE = ? AND PROD_COD2 = 'L5'
      `, [newStatus, user.username, company_code, registration_code, product_code]);
      
      console.log('ASSE_PROD 테이블 업데이트 완료');
      
      // 2. ASSE_PROT 이력 테이블 처리 (L5: 단종 상태 이력)
      const pGub1 = company_code;
      const pGub2 = registration_code;
      ...
      console.log('=== Product Registration Toggle Discontinued API 성공 완료 ===');
    ...
    console.error('=== Product Registration Toggle Discontinued API 에러 ===');
